...
38. 在软件开发中，散布在应用多处的功能被称为**横切关注点(cross-cutting concern)**，把这些横切关注点与业务逻辑相分离正是面向切面编程所要解决的问题。

39. 如果整个应用中都是用相同的基类，继承往往会导致一个脆弱的对象体系；使用委托机制则会导致对委托对象的复杂调用。

40. spring切面定义了5种类型的通知：
- a.前置通知(before):在调用目标方法之前调用通知功能；对应注解：@Before
- b.后置通知(after):在调用目标方法之后调用通知功能；对应注解：@After
- c.返回通知(After-returning):在目标方法成功返回后调用通知；对应注解：@AfterReturning
...
42. spring基于动态代理，所以spring只支持方法连接点。

43. 定义命名的切点：
```
@Pointcut("execution(** content.Performance.perform(..))")
